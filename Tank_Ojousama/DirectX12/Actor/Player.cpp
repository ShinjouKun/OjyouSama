...
void Player::UseWeapon1()
{
	...
	axis.normalize();
	mNormalAtkParticle->setPos(Vector3(position.x - (axis.x * 2.5f), position.y + 1.5f, position.z - (axis.z * 2.5f)));//後で調整
...
void Player::UseWeapon2()
{
	...
	if (modelChanger->GetWeaponState1() == WeaponsState::MachinGun)
	{
		...
		auto axis = CamVelocity;
		axis.normalize();
		mMGAParticle->setPos(Vector3(position.x - (axis.x * 2.5f), position.y + 1.5f, position.z - (axis.z * 2.5f)));//後で調整
		mMGAParticle->Play();
	}
